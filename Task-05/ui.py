"""Rich-based rendering: the stats header, the process table, the footer."""
...
# Fixed line-count overhead of everything that isn't a table data row:
# header panel (3) + table borders/column-header (4) + footer panel (4).
# Kept constant across frames so the visible-row budget doesn't jitter.
NON_TABLE_LINES = 11
...
def build_screen(cpu_percent, mem_used_kb, mem_total_kb, all_rows, selected_index, status_message, terminal_height):
    """Build the full frame, windowing the process table so the header and
    footer always stay on-screen even when the process list is taller than
    the terminal (the way htop scrolls rather than letting content clip)."""
